...
\input{../visualizations/tsms_flow}
...
The algorithm changes only in that the initial model fit includes random effects (\textit{lmer package})
...
Those intercepts are then used to update the parameters of $\Sigma_b$. A flowchart depicting the steps of the algorithm is given in 
...
inference, we now proceed to assess the algorithm's performance.


\subsection{To-Do List}
\begin{itemize}
    \item Amend code so that likelihood is split based on ybar and not uhat
    \item Fix Model Selection Flowchart
    \item ASSUMPTION OF INDEPENDENCE IN CALCULATING VARIANCE OF ESTIMATE
    \item Implications for STRICTLY STATIONARY SAMPLE SIZES aND NON-CONSTANT MEAN
    \item MATRIX N and how the likelihood changes 
    \item Comment on missing time steps
    \item MAKE SURE ALL NOTATION HAS BEEN DEFINED!!!!
    \item Formula for updating $\sigma_b^2$
\end{itemize}